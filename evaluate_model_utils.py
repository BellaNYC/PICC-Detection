...
def calculate_model_IOU(input_tensor, label_tensor):
    """
    calculate IOU = A and B / A or B
    :param input_tensor: binary segmentation logits
    :param label_tensor: binary segmentation label
    :return:
    """
    ...
    x = tf.cast(final_output, dtype=tf.bool)
    y = tf.cast(label_tensor, dtype=tf.bool)

    ind1 = tf.logical_and(tf.equal(x, y), tf.equal(x, True))
    ind1 = tf.cast(ind1, dtype=tf.int32)
    intersection = tf.reduce_sum(ind1)
    ind2 = tf.logical_or(x, y)
    ind2 = tf.cast(ind2, dtype=tf.int32)
    union = tf.reduce_sum(ind2)
