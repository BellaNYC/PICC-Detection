...
def init_args():
    ...
    parser.add_argument('-d', '--dataset_dir', type=str, default='./data/training_data_example',
...
def train_lanenet(dataset_dir, weights_path=None, net_flag='vgg'):
    ...
    with tf.device('/gpu:1'):
        ...
        train_iou = evaluate_model_utils.calculate_model_IOU(
            train_compute_ret['binary_seg_logits'], train_binary_labels
        )
        ...
        train_iou_scalar = tf.summary.scalar(
            name='train_iou', tensor=train_iou
        )
        ...
        train_merge_summary_op = tf.summary.merge(
            [train_accuracy_scalar, train_iou_scalar, train_binary_seg_loss_scalar, train_fn_scalar, train_fp_scalar]
        )
        ...
        val_iou = evaluate_model_utils.calculate_model_IOU(
            val_compute_ret['binary_seg_logits'], val_binary_labels
        )
        ...
        val_accuracy_iou = tf.summary.scalar(
            name='val_iou', tensor=val_accuracy
        )
        ...
        val_merge_summary_op = tf.summary.merge(
            [val_accuracy_scalar, val_accuracy_iou, val_binary_seg_loss_scalar, val_fn_scalar, val_fp_scalar]
        )
    ...
    train_loss_list, train_accuracy_list, train_iou_list, train_fn_list, train_fp_list = [], [], [], [], []
    val_loss_list, val_accuracy_list, val_iou_list, val_fn_list, val_fp_list = [], [], [], [], []
    with sess.as_default():
        ...
        for epoch in range(train_epochs):
            ...
            _, train_c, train_accuracy_figure, train_iou_figure, train_fn_figure, train_fp_figure, \
                ...
                sess.run([optimizer, train_total_loss, train_accuracy, train_iou, train_fn, train_fp,
            ...
            if epoch % CFG.TRAIN.DISPLAY_STEP == 0:
                log.info('Epoch: {:d} total_loss= {:6f} binary_seg_loss= {:6f} '
                         'instance_seg_loss= {:6f} accuracy= {:6f} iou= {:6f} fp= {:6f} fn= {:6f}'
                         ' lr= {:6f} mean_cost_time= {:5f}s '.
                         format(epoch + 1, train_c, train_binary_loss, train_instance_loss, train_accuracy_figure, train_iou_figure,
                                train_fp_figure, train_fn_figure, lr, np.mean(train_cost_time_mean)))
                train_cost_time_mean.clear()

            # validation part
            val_c, val_accuracy_figure, val_iou_figure, val_fn_figure, val_fp_figure, \
                ...
                sess.run([val_total_loss, val_accuracy, val_iou, val_fn, val_fp,
            ...
            if epoch % CFG.TRAIN.VAL_DISPLAY_STEP == 0:
                log.info('Epoch_Val: {:d} total_loss= {:6f} binary_seg_loss= {:6f} '
                         'instance_seg_loss= {:6f} accuracy= {:6f} iou= {:6f} fp= {:6f} fn= {:6f}'
                         ' mean_cost_time= {:5f}s '.
                         format(epoch + 1, val_c, val_binary_loss, val_instance_loss, val_accuracy_figure, val_iou_figure,
                                val_fp_figure, val_fn_figure, np.mean(train_cost_time_mean)))
                train_cost_time_mean.clear()

            if epoch % 100 == 0:
                train_loss_list.append(train_binary_loss)
                train_accuracy_list.append(train_accuracy_figure)
                train_iou_list.append(train_iou_figure)
                train_fn_list.append(train_fn_figure)
                train_fp_list.append(train_fp_figure)
                val_loss_list.append(val_binary_loss)
                val_accuracy_list.append(val_accuracy_figure)
                val_iou_list.append(val_iou_figure)
                val_fn_list.append(val_fn_figure)
                val_fp_list.append(val_fp_figure)

            if epoch % 2000 == 0:
                saver.save(sess=sess, save_path=model_save_path, global_step=global_step)
                np.savez('./tboard/logs.npz',
                         train_loss=np.array(train_loss_list),
                         train_accuracy=np.array(train_accuracy_list),
                         train_iou=np.array(train_iou_list),
                         train_fn=np.array(train_fn_list),
                         train_fp=np.array(train_fp_list),
                         val_loss=np.array(val_loss_list),
                         val_accuracy=np.array(val_accuracy_list),
                         val_iou = np.array(val_iou_list),
                         val_fn=np.array(val_fn_list),
                         val_fp=np.array(val_fp_list))
